...
describe ("Collecting tokens again", function() {
    it ("Repeat collection", async function() {
        ...
        */
        ...
        await ido.connect(clientZ).depositAssets(400,contractReceiveHash,signatureZ);
        usdcValueZ = await ido.connect(clientZ).viewPersonalAssets();
        expect (usdcValueZ).to.equal(400);
        usdcTotal = await ido.connect(clientY).viewTotalAssetsInContract();
        expect (usdcTotal).to.equal(2200);
        ...
        uncollectedX = await ido.connect(clientX).viewUncollectedTokens();
        //console.log("Calculated: " + Math.floor(shareX * sumSatisToken));
        expect (uncollectedX).to.equal(Math.floor(shareX * sumSatisToken));
        console.log("X-uncollected: " + uncollectedX);
        let shareY = 1000/2200;
        uncollectedY = await ido.connect(clientY).viewUncollectedTokens();
        expect (uncollectedY).to.equal(Math.floor(shareY * sumSatisToken));
        console.log("Y-uncollected: " + uncollectedY);
        let shareZ = 400/2200;
        ...
        await ido.connect(clientX).collectTokens();
        await ido.connect(clientY).collectTokens();
        await ido.connect(clientZ).collectTokens();
        uncollectedX = await ido.connect(clientX).viewUncollectedTokens();
        uncollectedY = await ido.connect(clientY).viewUncollectedTokens();
        uncollectedZ = await ido.connect(clientZ).viewUncollectedTokens();
        expect (uncollectedX).to.equal(0);
        expect (uncollectedY).to.equal(0);
